...
async def process_ingestion_job(
    ...
    if job.job_type == JobType.INGEST_RESOURCE:
        await _process_resource_ingestion(job, progress_callback)
...
async def _process_resource_ingestion(
    ...
    完整流程：Payload -> Parse -> Chunk -> Embed -> Upsert to Qdrant -> Send Result to Rust
