...
/// Task 表的完整字段列表（用于 SELECT 查询）
const TASK_FIELDS: &str = 
    "task_id, uuid, title, description, summary, status, done_date, priority, due_date, \
     created_at, updated_at, is_deleted, deleted_at, user_id";
...
pub async fn insert_task(pool: &DbPool, params: NewTask<'_>) -> Result<i64, sqlx::Error> {
    ...
    let result = sqlx::query(
        "INSERT INTO tasks (uuid, title, description, summary, status, priority, due_date, user_id) \
         VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
    )
    .bind(params.uuid)
    .bind(params.title)
...
pub async fn get_task_by_id(pool: &DbPool, task_id: i64) -> Result<TaskRecord, sqlx::Error> {
    ...
    let sql = format!("SELECT {} FROM tasks WHERE task_id = ?", TASK_FIELDS);
...
pub async fn list_active_tasks(pool: &DbPool) -> Result<Vec<TaskRecord>, sqlx::Error> {
    let sql = format!(
        "SELECT {} FROM tasks WHERE status = 'todo' AND is_deleted = 0 ORDER BY created_at DESC",
        TASK_FIELDS
    );
    sqlx::query_as::<_, TaskRecord>(&sql)
        .fetch_all(pool)
...
pub async fn list_all_tasks(pool: &DbPool) -> Result<Vec<TaskRecord>, sqlx::Error> {
    let sql = format!(
        "SELECT {} FROM tasks WHERE is_deleted = 0 AND due_date IS NOT NULL ORDER BY due_date ASC, priority DESC",
        TASK_FIELDS
    );
    sqlx::query_as::<_, TaskRecord>(&sql)
        .fetch_all(pool)
...
pub async fn list_tasks_by_date(pool: &DbPool, date: &str) -> Result<Vec<TaskRecord>, sqlx::Error> {
    let sql = format!(
        "SELECT {} FROM tasks WHERE DATE(due_date) = DATE(?) AND is_deleted = 0 ORDER BY due_date ASC, priority DESC",
        TASK_FIELDS
    );
    sqlx::query_as::<_, TaskRecord>(&sql)
        .bind(date)
...
pub async fn mark_task_as_done(pool: &DbPool, task_id: i64) -> Result<(), sqlx::Error> {
    sqlx::query(
        "UPDATE tasks SET status = 'done', done_date = CURRENT_TIMESTAMP, updated_at = CURRENT_TIMESTAMP \
...
pub async fn mark_task_as_todo(pool: &DbPool, task_id: i64) -> Result<(), sqlx::Error> {
    sqlx::query(
        "UPDATE tasks SET status = 'todo', done_date = NULL, updated_at = CURRENT_TIMESTAMP \
...
pub async fn update_task_priority(pool: &DbPool, task_id: i64, priority: TaskPriority) -> Result<(), sqlx::Error> {
    sqlx::query(
        "UPDATE tasks SET priority = ?, updated_at = CURRENT_TIMESTAMP \
...
pub async fn update_task_due_date(pool: &DbPool, task_id: i64, due_date: Option<&str>) -> Result<(), sqlx::Error> {
    sqlx::query(
        "UPDATE tasks SET due_date = ?, updated_at = CURRENT_TIMESTAMP \
         WHERE task_id = ? AND is_deleted = 0",
    )
    .bind(due_date)
    .bind(task_id)
...
pub async fn update_task_title(pool: &DbPool, task_id: i64, title: &str) -> Result<(), sqlx::Error> {
    sqlx::query(
        "UPDATE tasks SET title = ?, updated_at = CURRENT_TIMESTAMP \
         WHERE task_id = ? AND is_deleted = 0",
    )
    .bind(title)
    .bind(task_id)
...
/// 更新任务摘要 (AI 生成)
pub async fn update_task_summary(pool: &DbPool, task_id: i64, summary: Option<&str>) -> Result<(), sqlx::Error> {
    sqlx::query(
