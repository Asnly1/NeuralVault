...
pub async fn update_chat_message_contents(
    ...
    let (input_tokens, output_tokens, reasoning_tokens, total_tokens) = usage
...
pub async fn delete_chat_message(pool: &DbPool, message_id: i64) -> Result<(), sqlx::Error> {
    sqlx::query!("DELETE FROM chat_messages WHERE message_id = ?", message_id)
        .execute(pool)
        .await?;
...
pub async fn delete_message_attachment(
    ...
    sqlx::query!(
        "DELETE FROM message_attachments WHERE message_id = ? AND node_id = ?",
        message_id,
        node_id,
    )
        .execute(pool)
...
pub async fn set_session_bindings(
    ...
    sqlx::query!(
        "DELETE FROM session_bindings WHERE session_id = ? AND binding_type = ?",
        session_id,
        binding_type,
    )
        .execute(pool)
