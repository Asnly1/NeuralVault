...
mod tests {
    ...
    fn placeholder_provider_config() -> ProviderConfig {
        ProviderConfig {
    ...
    fn create_integration_test_provider_config() -> Option<ProviderConfig> {
        let api_key = env::var("NEURALVAULT_GEMINI_API_KEY")
    ...
    fn require_integration_test_provider_config(test_name: &str) -> Option<ProviderConfig> {
        let config = create_integration_test_provider_config();
        if config.is_none() {
            println!(
                "Skipping {test_name}: set NEURALVAULT_GEMINI_API_KEY or GEMINI_API_KEY to run network integration tests"
            );
        }
        config
    }

    fn integration_test_file_path(var_name: &str) -> Option<String> {
    ...
    #[tokio::test]
    #[ignore = "requires network access and a configured Gemini API key"]
    async fn test_stream_chat_simple_message() {
        let Some(provider_config) =
            require_integration_test_provider_config("test_stream_chat_simple_message")
        else {
            return;
        };
    ...
    #[tokio::test]
    #[ignore = "requires network access, a configured Gemini API key, and a test image"]
    async fn test_stream_chat_with_image() {
        let Some(provider_config) =
            require_integration_test_provider_config("test_stream_chat_with_image")
        else {
            return;
        };
        let Some(test_image_path) = integration_test_file_path("NEURALVAULT_TEST_IMAGE_PATH") else {
            println!("Skipping test_stream_chat_with_image: set NEURALVAULT_TEST_IMAGE_PATH to run this test");
            return;
        };

        if !Path::new(&test_image_path).exists() {
            println!(
                "Skipping test_stream_chat_with_image: test image not found at {}",
                test_image_path
            );
            return;
        }
        ...
        message.images = vec![test_image_path];
    ...
    async fn test_generate_structured_json_simple() {
        let Some(provider_config) =
            require_integration_test_provider_config("test_generate_structured_json_simple")
        else {
            return;
        };
        let service = LlmService::new();
    ...
    async fn test_generate_structured_json_with_image() {
        let Some(provider_config) =
            require_integration_test_provider_config("test_generate_structured_json_with_image")
        else {
            return;
        };
        let Some(test_image_path) = integration_test_file_path("NEURALVAULT_TEST_IMAGE_PATH") else {
            println!("Skipping test_generate_structured_json_with_image: set NEURALVAULT_TEST_IMAGE_PATH to run this test");
            return;
        };

        if !Path::new(&test_image_path).exists() {
            println!(
                "Skipping test_generate_structured_json_with_image: test image not found at {}",
                test_image_path
            );
            return;
        }
    ...
    #[tokio::test]
    #[ignore = "requires network access, a configured Gemini API key, and a test PDF"]
    async fn test_generate_structured_json_with_pdf() {
        let Some(provider_config) =
            require_integration_test_provider_config("test_generate_structured_json_with_pdf")
        else {
            return;
        };
        let Some(test_pdf_path) = integration_test_file_path("NEURALVAULT_TEST_PDF_PATH") else {
            println!("Skipping test_generate_structured_json_with_pdf: set NEURALVAULT_TEST_PDF_PATH to run this test");
            return;
        };

        if !Path::new(&test_pdf_path).exists() {
            println!(
                "Skipping test_generate_structured_json_with_pdf: test PDF not found at {}",
                test_pdf_path
            );
            return;
        }
    ...
    async fn test_stream_chat_unsupported_provider() {
        let service = LlmService::new();
        let provider_config = placeholder_provider_config();
